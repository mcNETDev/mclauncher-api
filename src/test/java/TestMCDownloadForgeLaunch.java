...
import sk.tomsik68.mclauncher.impl.versions.mcdownload.MCDownloadOnlineForgeVersionList;
...
public class TestMCDownloadForgeLaunch {
	...
	public void test() {
		...
		try {
			...
			IVersion toLaunch = versionList.retrieveVersionInfo("1.7.10-10.13.4.1614-1.7.10");
			System.out.println(toLaunch);
			try {
				toLaunch.getInstaller().install(toLaunch, mc, null);
				List<String> launchCommand = toLaunch.getLauncher().getLaunchCommand(session, mc, null, toLaunch,
						new ILaunchSettings() {
							...
							public List<String> getJavaArguments() {
								return Arrays.asList("-XX:+UseConcMarkSweepGC", "-XX:+CMSIncrementalMode",
