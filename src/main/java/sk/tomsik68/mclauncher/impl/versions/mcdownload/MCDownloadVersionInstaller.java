...
final class MCDownloadVersionInstaller implements IVersionInstaller {
		...
		if (version.getJarVersion().equals(version.getId())) {
			...
			try {
				...
				if (v.isForge()) {
					...
					url = JAR_DOWNLOAD_URL.replaceAll("<VERSION>", version.getId());
				}
	...
	private void updateResources(MinecraftInstance mc, MCDownloadVersion version, IProgressMonitor progress)
			throws Exception {
